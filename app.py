...
with lpan:
    ...
    with st_capture(output5.code):
        ...
        print("{} | 農曆︰{} | {} |\n{} |\n{} |\n{} - {} - {} ({}) | {} | \n積{}數︰{} | \n紀元︰{} | 主筭︰{}  客筭︰{} |\n{}禽值年 | {}門值事 | \n{}卦值年 | 皇極經世值年卦︰{} | 陽九︰{} 百六︰{} |\n{}{}{}\n{} \n\n".format(config.gendatetime(y,m,d,h), lunard, jieqi.jq(y,m,d,h),  gz, config.kingyear(y), config.ty_method(tn), config.taiyi_name(num), ty.kook(num, tn).get("文"),  ttext.get("局式").get("年"), config.five_zi_yuan(ty.kook(num, tn).get("數"), ttext.get("干支")[0]) , config.taiyi_name(num)[0],tynum, ttext.get("紀元"), homecal, awaycal, yc, ed, g, config.wangji_yeargua.get(config.lunar_date_d(y, m, d).get("年")), yj, bl, ty.ty_gong_dist(num, tn), ty.threedoors(num, tn), ty.fivegenerals(num, tn), ty.geteightdoors(num, tn) ))
    ...
    with st_capture(output5.code):
        try:
            if manual:
                ...
                st.title("九宮分野︰")
                st.image("pic/太乙九宮分野圖.jpg", use_column_width=True)
                print("{} | 農曆︰{} | {} |\n{} |\n{} |\n{} - {} - {} ({}) | {} | \n積{}數︰{} | \n紀元︰{} | 主筭︰{}  客筭︰{} |\n{}禽值年 | {}門值事 | \n{}卦值年 | 皇極經世年卦︰{} | 陽九︰{} 百六︰{} |\n{}{}{}\n{} \n\n".format(config.gendatetime(y,m,d,h), lunard, jieqi.jq(y,m,d,h),gz, config.kingyear(y), config.ty_method(tn), config.taiyi_name(num), ty.kook(num, tn).get("文") ,text.get("局式").get("年"), config.five_zi_yuan(ty.kook(num, tn).get("數"), ttext.get("干支")[0]) , config.taiyi_name(num)[0],tynum, ttext.get("紀元"), homecal, awaycal, yc, ed, g, config.wangji_yeargua.get(config.lunar_date_d(y, m, d).get("年")),yj, bl, ty.ty_gong_dist(num, tn), ty.threedoors(num, tn), ty.fivegenerals(num, tn), ty.geteightdoors(num, tn) ))
                expander = st.expander("原始碼")
                expander.write(str(ttext))
            else:
                st.empty()
