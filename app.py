        ...
        try:
            if manual:
                ...
                if num == 5:
                    ...
                    sj_su_predict = f"始擊落{ty.sf_num(0, 0)}宿，{su_dist.get(ty.sf_num(0, 0))}"
                    tg_sj_su_predict = config.multi_key_dict_get(tengan_shiji, config.gangzhi(my, mm, md, mh, mmin)[0][0]).get(config.Ganzhiwuxing(ty.sf(0, 0)))
                    three_door = ty.threedoors(0, 0)
                    five_generals = ty.fivegenerals(0, 0)
                    home_vs_away1 = ty.wc_n_sj(0, 0)
            ...
            if num != 5:
                print(f"{config.gendatetime(y, m, d, h, min)} | 積{config.taiyi_name(num)[0]}數︰{tynum} | \n農曆︰{lunard} | {jieqi.jq(y, m, d, h, min)} |\n{gz} |\n{config.kingyear(y)} |\n{config.ty_method(tn)} - {config.taiyi_name(num)} - {ty.kook(num, tn).get('文')} ({ttext.get('局式').get('年')}) | \n紀元︰{ttext.get('紀元')} | 主筭︰{homecal} 客筭︰{awaycal} 定筭︰{setcal} |\n{yc}禽值年 | {ed}門值事 | \n{g}卦值年 | 太乙統運卦︰{config.find_gua(config.lunar_date_d(y, m, d).get('年'))} |")
            if num == 5:
                print(f"{config.gendatetime(y, m, d, h, min)} {zhao} - {ty.taiyi_life(sex_o).get('性別')} - {config.taiyi_name(0)[0]} - {tynum} | \n農曆︰{lunard} | {jieqi.jq(y, m, d, h, min)} |\n{gz} |\n{config.kingyear(y)} |\n{ty.kook(0, 0).get('文')} ({ttext.get('局式').get('年')}) | \n紀元︰{ttext.get('紀元')} | 主筭︰{homecal} 客筭︰{awaycal} |\n{yc}禽值年 | {ed}門值事 | \n{g}卦值年 | 太乙統運卦︰{config.find_gua(config.lunar_date_d(y, m, d).get('年'))}")
