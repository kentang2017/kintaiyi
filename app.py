    ...
    with st_capture(output5.code):
        ...
        try:
            if manual:
                ...
                st.write(f'<object type="image/svg+xml" data="data:image/svg+xml,{genchart}"></object>', width=400, height=400)
                print("{} | 農曆︰{} | {} |\n{} |\n{} |\n{} - {} - {} ({}) | {} | \n積{}數︰{} | \n紀元︰{} | 主筭︰{} 客筭︰{} |\n{}禽值年 | {}門值事 | \n{}卦值年 | 太乙統運卦︰{} | \n陽九︰{} 百六︰{} |\n{}{}{}\n{} \n\n".format(config.gendatetime(my,mm,md,mh), lunard, jieqi.jq(my,mm,md,mh,mmin),gz, config.kingyear(my), config.ty_method(tn), config.taiyi_name(num), ty.kook(num, tn).get("文") ,ttext.get("局式").get("年"), ttext.get("五子元局") , config.taiyi_name(num)[0],tynum, ttext.get("紀元"), homecal, awaycal, yc, ed, g, config.find_gua(config.lunar_date_d(my, mm, md).get("年")) , yj, bl, ty.ty_gong_dist(num, tn), ty.threedoors(num, tn), ty.fivegenerals(num, tn), ty.geteightdoors_text(num, tn) ))
            else:
                output5 = st.empty()
        except ValueError:
            st.empty()
        ...
        if instant:
            ...
            st.image("pic/太乙九宮分野圖.jpg", use_column_width=True)
            st.write(f'<object type="image/svg+xml" data="data:image/svg+xml,{genchart}"></object>', width=400, height=400)
            print("{} | 農曆︰{} | {} |\n{} |\n{} |\n{} - {} - {} ({}) |  {} | \n積{}數︰{} | \n紀元︰{} | 主筭︰{}  客筭︰{} |\n{}禽值年 | {}門值事 | \n{}卦值年 | 太乙統運卦︰{} | \n陽九︰{} 百六︰{}  | \n{}{}{}\n{} \n\n".format(config.gendatetime(y,m,d,h), lunard, jieqi.jq(y,m,d,h,min),  gz, config.kingyear(y), config.ty_method(tn), config.taiyi_name(num), ty.kook(num, tn).get("文"),  ttext.get("局式").get("年"), ttext.get("五子元局") , config.taiyi_name(num)[0],tynum, ttext.get("紀元"), homecal, awaycal, yc, ed, g, config.find_gua(config.lunar_date_d(y, m, d).get("年")),   yj, bl, ty.ty_gong_dist(num, tn), ty.threedoors(num, tn), ty.fivegenerals(num, tn), ty.geteightdoors_text(num, tn) ))
