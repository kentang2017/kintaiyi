...
def _to_julian_day(year, month, day, hour=0, minute=0):
    """Julian Day 計算（用於天數差），與前端 taiyi-jieqi.js 的 toJulianDay 一致"""
    y = year
    m = month
    if m <= 2:
        y -= 1
...
def shichen_ju(year, month, day, hour):
    ...
    if not xz or not dz or not dz_prev:
        return None
    cur_jd = _to_julian_day(year, month, day, hour, 0)
    xz_jd = _to_julian_day(xz[0], xz[1], xz[2], 0, 0)
    dz_jd = _to_julian_day(dz[0], dz[1], dz[2], 0, 0)
    dz_prev_jd = _to_julian_day(dz_prev[0], dz_prev[1], dz_prev[2], 0, 0)
    if cur_jd >= xz_jd and cur_jd < dz_jd:
        ...
        base, dun = dz, '陽遁'
    base_jd = _to_julian_day(base[0], base[1], base[2], 0, 0)
    days = cur_jd - base_jd
    hour_num = (hour + 1) // 2
...
if __name__ == '__main__':
    #print(find_lunar_minute("甲子"))
    print(gangzhi(2025,9,12,14,4))
