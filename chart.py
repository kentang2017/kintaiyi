    ...
    for layer_idx, divs in enumerate(num_divisions):
        ...
        for div in range(divs):
            ...
            _draw_sector(layer, start, end, inner, outer, raw,
                         ...
                         is_fifth_layer=(layer_idx == 3))  # 第五層
        d.append(layer)
...
if __name__ == "__main__":
    ...
    with open("test_hour_fifth_layer.svg", "w", encoding="utf-8") as f:
        f.write(svg_hour)
    print("已產生 test_hour_fifth_layer.svg（第五層關鍵字上色）")
    ...
    svg_life = gen_chart_life(
        ...
        sixth_layer=[
            ['巳','太乙','楚'], ['午','飛符','荊州'], ['未','主大','秦'], ['坤','五福','梁州'],
            ['申','直符','晉'], ['酉','客參','趙雍'], ['戌','地乙','魯'], ['乾','四神','冀州'],
            ['亥','其他','衛'], ['子','其他','齊'], ['丑','其他','吳'], ['寅','其他','燕']
        ]
    )
    with open("test_life_fifth_layer.svg", "w", encoding="utf-8") as f:
        f.write(svg_life)
    print("已產生 test_life_fifth_layer.svg（第五層關鍵字上色）")
