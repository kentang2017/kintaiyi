...
FIFTH_LAYER_KEYWORDS = {
    '太乙': 'green', '客參': 'green',           # 木 → 綠字
    '飛符': 'red',   '始擊': 'red', '直符': 'red', '計神': 'red',     # 火 → 紅字
    '主大': 'gold',  '天乙': 'gold', '大游': 'gold',                 # 金 → 金字
    '主參': 'blue',  '客大': 'blue', '小游': 'blue', '四神': 'blue', # 水 → 藍字
    '五福': 'brown', '文昌': 'brown', '地乙': 'brown',
    '君基': 'brown', '臣基': 'brown', '民基': 'brown',               # 土 → 棕字
}
# =========================================================
...
def _draw_sector(group, start, end, inner, outer, raw_label,
    ...
    if is_second_layer:
        ...
        fill = BRANCH_COLORS.get(GATE_TO_BRANCH[key], 'gray') if key else 'gray'
    elif is_third_layer:
    ...
    # ---- 文字色（第五層強制覆蓋）----
    text_fill = TEXT_COLORS.get(fill, 'white')  # 預設對比色
    if is_fifth_layer:
        label_str = _format_label(raw_label)
        for keyword, color in FIFTH_LAYER_KEYWORDS.items():
            if keyword in label_str:
                text_fill = color  # 強制用關鍵字顏色
                break
...
if __name__ == "__main__":
    ...
    svg = gen_chart_hour(
        ...
        sixth_layer=[
            ['巳','太乙','楚'], ['午','飛符','荊州'], ['未','主大','秦'], ['坤','五福','梁州'],
            ['申','直符','晉'], ['酉','客參','趙雍'], ['戌','地乙','魯'], ['乾','四神','冀州'],
            ['亥','大游','衛'], ['子','計神','齊兗'], ['丑','文昌','吳'], ['艮','主參','青州'],
            ['寅','客大','燕'], ['卯','小游','徐州'], ['辰','始擊','鄭'], ['巽','君基','揚州']
        ],
        twentyeight=constellations,
        degrees=[360/28]*28,
        rotate_28=0
    )
    with open("test_fifth_layer_text_color.svg", "w", encoding="utf-8") as f:
        f.write(svg)
    print("已產生 test_fifth_layer_text_color.svg（第五層文字上色，背景不變）")
